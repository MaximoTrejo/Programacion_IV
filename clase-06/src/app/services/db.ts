...
export class Db {
  private supService = inject(Supabase);
async traerAutos() {
    ...
    const resultado = await this.supService.supabase
      .schema('public')
      .from('autos')
  ...
  async crearAuto(auto: Auto): Promise<any | null> {
    const resultado = await this.supService.supabase.from('autos').insert(auto);

    return resultado.error?.message;
  }

  async traerAutoPorId(id: number) {
    const { data, error } = await this.supService.supabase.from('autos').select('*').eq('id', id);
  }

  async modificarAuto(auto: Auto) {
    const { data, error } = await this.supService.supabase.from('autos').update(auto).eq('id', auto.id);
  }

  async eliminarAuto(id: number) {
    const { data, error } = await this.supService.supabase.from('autos').delete().eq('id', id);
  }
